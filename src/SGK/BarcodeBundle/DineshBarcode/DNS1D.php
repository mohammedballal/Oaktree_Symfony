...
//    }
    ...
    public function getBarcodeHTML($code, $type, $w = 2, $h = 30, $color = 'black') {
        $this->setBarcode($code, $type);
        $html = '<div style="margin: 0 auto;font-size:0;position:relative;">' . "\n";
        $html = '<div style="margin: 0 auto;font-size:0;position:relative;width:' . ($this->barcode_array['maxw'] * $w) . 'px;height:' . ($h) . 'px;">' . "\n";
        // print bars
        $x = 0;
        foreach ($this->barcode_array['bcode'] as $k => $v) {
            $bw = round(($v['w'] * $w), 3);
